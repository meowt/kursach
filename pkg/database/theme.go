...
func (t *Theme) GetByID(id string) error {
	err := db.QueryRow(fmt.Sprintf("SELECT * FROM themes WHERE id = '%s' ;", id)).Scan(
		&t.Path,
		&t.ReleaseDate,
		&t.CreatorName,
		&t.Description,
...
func (t *Theme) GetWithLimit(limit int) error {
	err := db.QueryRow(fmt.Sprintf("SELECT * FROM themes WHERE id = '%v' ;", limit)).Scan(
		&t.Path,
		&t.ReleaseDate,
		&t.CreatorName,
		&t.Description,
...
func GetLastThemeID() int {
	var id int
	_ = db.QueryRow(fmt.Sprintf("SELECT max(id) FROM themes;")).Scan(&id)
	return id
}
...
func GetCreatorsThemes(creator string) ([]Theme, error) {
	...
	rows, err := db.Query(fmt.Sprintf("SELECT * FROM themes WHERE creator_name = '%s' LIMIT 4;", creator))
	if err != nil {
...
func GetLastThemes() ([]Theme, error) {
	...
	rows, err := db.Query("SELECT * FROM themes ORDER BY release_date LIMIT 4;")
	if err != nil {
		fmt.Println(err.Error())
	}
	for rows.Next() {
