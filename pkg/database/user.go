...
func (user *User) GetPageData(d string) error {
	...
	err := db.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE username = '%s' ;", d)).Scan(&user.Username, &user.Email, &user.Password, &user.Description, &user.ID)
	if err != nil {
...
func (user *User) LoginRequest(email, password string) string {
	...
	err := db.QueryRow(sqlQuery).Scan(&passwordCheck)
	...
	if service.ComparePassword(password, passwordCheck) {
		...
		err = db.QueryRow(sqlQuery).Scan(&user.Username, &user.Email, &user.Password, &user.Description, &user.ID)
		return ""
	} else {
...
func (user *User) RegRequest() string {
	...
	var scanStruct User
	row := db.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE email = '%s' LIMIT 1;", user.Email))
	...
	//Checking of existing users with this email
	row = db.QueryRow(fmt.Sprintf("SELECT * FROM users WHERE username = '%s' LIMIT 1;", user.Username))
	e = row.Scan(&scanStruct.Username, &scanStruct.Email, &scanStruct.Password, &scanStruct.Description, &scanStruct.ID)
	if e != nil && e != sql.ErrNoRows {
		return "error"
	}
